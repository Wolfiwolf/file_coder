...
{
    int p = string.size() - 1;
    for(int i = 0; i != p && i-1 != p; i++)
    {
        std::swap(string[i], string[p]);
        p--;
    }
}
...
bool encode(const std::string& filepath)
{
    ...
    reverse_string(file_type);

    out_file << file_type.size();
    out_file << file_type;
...
bool decode(const std::string& filepath)
{
    ...
    in_file.read(buffer, file_size-file_type.size() - 1);
    in_file.close();
    ...
    out_file.write(&buffer[5], file_size- 5 - file_type.size() - 1);
